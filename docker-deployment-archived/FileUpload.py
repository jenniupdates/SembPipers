    ...
    try:
        ...
        if "input" in table:
            table_cols = requests.request(
                "GET", f"http://inputtable:5003/getInputTableColInfo?table={table}")
        else:
            table_cols = requests.request(
                "GET", f"http://configtable:5002/getConfigTableColInfo?table={table}")
        ...
        for key in df_dict.keys():
            ...
            validation_result = requests.post("http://processvalidation:5011/processActivatedRules",headers=headers,json=data)
            ...
            for line in file:
                ...
                if "def" in line:
                    if function_code != "":
                        ...
                        result = requests.post('http://genericfunctions:5007/insertFunction',json=body)
