...
CREATE TABLE IF NOT EXISTS created_formula_data (
    time_stamp datetime,
    internal_name varchar(20),
    formula_value float,
    primary key (time_stamp, internal_name)
);
...
CREATE TABLE IF NOT EXISTS manipulation_formula (
...
CREATE TABLE IF NOT EXISTS activated_manipulation_formula (
    formula_used varchar(64) not null,
    internal_name varchar(64) not null unique,
    column_variables varchar(100) not null,
    formula_python varchar(200) not null,
    primary key (internal_name),
    foreign key (formula_used)
    references manipulation_formula (formula_name)
);